# Crown Draughts — Product and Game Specification
...
Move diagonal pieces, make mandatory captures, chain jumps, and crown pieces that reach the far edge.
...
The repeated loop is: **inspect legal moves, choose a move or capture chain, resolve promotion, and pass the turn**. Input is locked only during the shortest non-interruptible resolution phase. Cosmetic animation may continue after the logical state is ready, but skip/fast-forward must settle every object into the exact deterministic end state.

### Scoring and victory

Win by removing or immobilizing the opponent; support draw rules and rating. Results show a component breakdown rather than one unexplained total. Store integers for score and simulation units; format values only in presentation. Ties use, in order: primary objective completion, fewer invalid actions, lower authoritative elapsed time, then stable session identifier.
...
- Ship a browser distribution with `starhermit.txt` at its root, `name=Crown Draughts`, and `launch=index.html`. Keep source files, secrets, design documents, and source maps outside the uploaded distribution.
